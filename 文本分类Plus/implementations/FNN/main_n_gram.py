...
class FNN(object):
    ...
    def __init__(self, layer_sizes, num_calsses):
        ...
        with tf.name_scope('loss'):
            self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.scores, labels = self.input_y, name = 'loss'))
...
argv_names = ['n', 'layer_sizes', 'learning_rate', 'dropout_keep_prob', 'batch_size', 'epochs']
print(sys.argv)
argv = sys.argv[1:]
