...
class CNN(object):
    ...
    def __init__ (self, seq_length, num_classes, dict_size, \
                  embedding_size, filter_sizes, num_filters, regularization_rate):
        ...
        with tf.name_scope('loss'):
            self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.scores, labels = self.input_y)) + self.regular_loss
...
argv_names = ['train_file_name','test_file_name','dict_size','embedding_size',\
              'filter_sizes','num_filters','dropout_keep_prob', 'regularization_rate',\
              'batch_size','epochs']
FLAGS = {}
print(sys.argv)
argv = sys.argv[1:]
...
test_x = np.array(test_x, np.int32)
...
cnn = CNN(seq_length = train_x.shape[1], num_classes = 5, dict_size = FLAGS['dict_size'], \
          ...
          num_filters = FLAGS['num_filters'], regularization_rate = FLAGS['regularization_rate'])
